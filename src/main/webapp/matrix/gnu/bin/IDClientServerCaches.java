 ...
 * @author admin
 */
public class IDClientServerCaches {
    
    private final InitialContext ic;
    private final Map<String, Object> cache;
    
    private static IDClientServerCaches me;
    
    static {
        try {
            me = new IDClientServerCaches();
        } catch (NamingException se) {
            throw new RuntimeException(se);
        }
    }
    
    private IDClientServerCaches() throws NamingException {
        ic = new InitialContext();
        cache = Collections.synchronizedMap(new HashMap<>());
    }
    
    public static IDClientServerCaches getInstance() {
        return me;
    }
